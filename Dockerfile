...
RUN sed -e "s/[0-9a-f]\{8\}-[0-9a-f]\{4\}-[0-9a-f]\{4\}-[0-9a-f]\{4\}-[0-9a-f]\{12\}/$(uuidgen)/g" conf/fedora-release-autotest.toml > my.toml
RUN pip install --force-reinstall  .
RUN pip install resultsdb_conventions
ENTRYPOINT ["fedora-messaging", "--conf", "my.toml", "consume"]
